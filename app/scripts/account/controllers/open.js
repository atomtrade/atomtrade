...
['$scope', 'wdAccount', '$timeout', '$location', 'wdStorage', '$window',
function ($scope, wdAccount, $timeout, $location, wdStorage, $window) {
    $scope.loading = true;
    ...
    function checkIdNo() {
        if (!$scope.userInfo.id_no) {
            $scope.userInfo.uiIdNoError = '请填写「身份证号码」';
            return false;
        } else if (!isCnNewID($scope.userInfo.id_no)) {
            $scope.userInfo.uiIdNoError = '身份证号有误，请检查';
            return false;
        } else {
